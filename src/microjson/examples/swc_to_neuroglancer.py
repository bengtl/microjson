...
Usage:
    python -m microjson.examples.swc_to_neuroglancer neuron.swc

    # Multiple SWC files:
    python -m microjson.examples.swc_to_neuroglancer neuron1.swc neuron2.swc neuron3.swc

    # Export only (no server):
    python -m microjson.examples.swc_to_neuroglancer neuron.swc --no-serve

    # Custom port:
    python -m microjson.examples.swc_to_neuroglancer neuron.swc --port 8080
...
def main() -> None:
    ...
    if args.no_serve:
        ...
        print(f"  python -m microjson.examples.neuroglancer_serve {output_dir}")
        return
