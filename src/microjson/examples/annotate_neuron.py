"""Annotate a neuron morphology in 3D using MicroJSON.

Demonstrates how to use the MicroJSON data model to:
  ...
  4. Export everything as a single MicroFeatureCollection
  5. View in Neuroglancer with skeleton + annotations overlaid

Usage:
    python -m microjson.examples.annotate_neuron tests/fixtures/sample_neuron.swc

    # With your own SWC file:
    python -m microjson.examples.annotate_neuron path/to/neuron.swc
"""
...
def find_soma(morphology: NeuronMorphology) -> list[MicroFeature]:
...
def find_branch_points(morphology: NeuronMorphology) -> list[MicroFeature]:
...
def find_terminals(morphology: NeuronMorphology) -> list[MicroFeature]:
...
# ---------------------------------------------------------------------------
# Build the annotated MicroFeatureCollection
# ---------------------------------------------------------------------------

def annotate_neuron(swc_path: str) -> MicroFeatureCollection:
    """Load SWC and build a fully annotated MicroFeatureCollection.
    ...
    features: list[MicroFeature] = [neuron_feature]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Annotate a neuron morphology in 3D using MicroJSON",
    )
    ...
    parser.add_argument(
        "--save-json",
        action="store_true",
        help="Also save the MicroJSON collection as JSON",
    )
    ...
    # Optional: save the raw MicroJSON
    if args.save_json:
