...
Usage:
    python -m microjson.examples.annotate_by_type path/to/neuron.swc

    # Export only (no server):
    python -m microjson.examples.annotate_by_type neuron.swc --no-serve

    # Save the MicroJSON collection as JSON:
    python -m microjson.examples.annotate_by_type neuron.swc --save-json
"""
...
def main() -> None:
    ...
    parser.add_argument("--save-json", action="store_true",
                        help="Save the MicroJSON collection as JSON")
    ...
    seg_features: list[MicroFeature] = []
    seg_ids: list[int] = []
    ...
    for segment_id, (swc_type, subtree) in enumerate(sorted(subtrees.items()), start=1):
        ...
        seg_features.append(MicroFeature(
            type="Feature",
