...
Public API:
    swc_to_microjson(swc_path) -> MicroFeature(geometry=TIN)
    swc_to_tin(swc_path, ...) -> MicroFeature(geometry=TIN)
...
from .model import MicroFeature, MicroFeatureCollection, TIN
...
class NeuronMorphology(BaseModel):
    ...
    directly in a MicroFeature. Use ``swc_to_microjson()`` or
    ``swc_to_tin()`` to get a MicroFeature with TIN geometry.
    """
...
# ---------------------------------------------------------------------------
# Internal SWC parser (returns NeuronMorphology — not for MicroFeature)
# ---------------------------------------------------------------------------
...
def swc_to_microjson(
    ...
    *,
    ...
    """Parse an SWC file and return a MicroFeature with TIN geometry.
    ...
    Returns:
        A MicroFeature wrapping a TIN geometry.
    """
    return swc_to_tin(
        swc_path,
        segments=segments,
...
def swc_to_tin(
    ...
    *,
    ...
    return MicroFeature(type="Feature", geometry=tin, properties={})
...
def swc_to_feature_collection(
    ...
    *,
    ...
    Returns:
        A MicroFeatureCollection with one Feature per SWC compartment type.
    """
